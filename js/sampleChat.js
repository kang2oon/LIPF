...
function createChatRoom(){
    ...
    $.ajax({
        url: host + '/itvApi/itv/'+interviewerSeq+'/chatroom/createForMobileApp?counselCategoryId='+counselCategoryId,
        type: 'GET',
        data: null, //JSON.stringify(chatroom),
        processData: false,
...
function getChatRooms(){
    ...
    $.ajax({
        url: host + '/itvApi/itv/'+interviewerSeq+'/roomsForMobileApp',
        type: 'GET',
        data: null, //JSON.stringify(chatroom),
        processData: false,
        contentType: 'application/json',
        beforeSend: function (xhr){
            xhr.setRequestHeader('x-auth-token', authToken);  //인증 토큰 설정
        },
        success: function (response) {
            console.log('create success');
            console.log(response);

            chatRoom = response.openRoom;
            //alert(chatRoom);

            if(typeof chatRoom == 'undefined'  ){
                alert('대화중인 채팅방이 없습니다. '); //열려있는 채팅 방이 없으므로 새로 생성
                $("#createDiv").show();
            }
            else{
                chatroomSeq = chatRoom.chatroomSeq;
...
function getChatMsgList(){ // 이전 메시지 조회
    ...
    $.ajax({
        url: host + '/itvApi/itv/'+interviewerSeq+'/chatroom/'+chatroomSeq+'/msgs/page?pageNum='+pageNum+'&pageSize=10000&direction=desc&lastEvtDtm=0&lastMsgSeq=0',
        type: 'GET',
        data: null,
        processData: false,
        //contentType: false,
